...
(**)
(*(* Definiamo un bottone per svuotare la tabella e riiniziare il gioco con un altro giocatore pulendo le celle di output create. Per ricominciare bisogner\[AGrave] rievalutare la cella che contiene YAHTZEE *)*)
...
(**)
(*(*Le variabili di gioco vengono inizalizzate per una nuova partita*)*)
...
(**)
(*(*Bottone per il salvataggio. Diventa cliccabile solo dopo aver giocato una partita intera, ovvero quando tutte le combinazioni sono state riempite. Quando il pulsante viene cliccato, la funzione al suo interno crea il file punteggi.txt se non esiste e aggiunge il punteggio realizzato al file. Se il file esiste gi\[AGrave] basta semplicemente aggiungervi il punteggio. La funzione funzCreaStringa che fa la maggior parte del lavoro verr\[AGrave] spiegata sotto alla sua definizione.*)*)
...
(**)
(*(*questa funzione prende come parametri una lista composta da n elementi ordinati in formato {punteggio(int), - ,nome(str)} e un punteggio nella forma {punteggio(int), - ,nome(str)} che sarebbe l'elemento da inserire nella lista. Restituisce una lista ordinata di n elementi ordinati in formato {punteggio(int), - ,nome(str)} aggiornata con il singolo punteggio passato in input. Tale risultato verr\[AGrave] utilizzato per mostarare high scores *)*)
