...
(*(* :History: Versions 20, Giuly a.a 2021/2022*)*)
...
(**)
(*(*usages*)*)
(*YAHTZEE::usage="YAHTZEE permetti di cominciare una partita da giocatore singolo di Yahtzee."*)
(**)
(*Begin["Private`"]*)
(**)
(*(*Definiamo il costruttore del gioco e tutti i sui moduli logici e grafici*)*)
...
(**)
(*(*Utilizziamo un seme per il random*)*)
(*SeedRandom[seme];*)
(**)
(*(*Importiamo le immagini per i dadi*)*)
...
(**)
(*(*Instanziamo le liste delle varie combinazioni: una completa, una per la sezione superiore (che assegna anche punti bonus) e una per la sezione inferiore del tabellone*)*)
...
(**)
(*(*Questa flag serve per selezionare solo una combinazione per turno*)*)
(*flagPunteggioSelezionato=False;*)
(**)
(*(*Questa flag permette di salvare il punteggio una volta sola a fine partita*)*)
(*flagSave = True;*)
(**)
(*(*Colori dei dadi al lancio (red/green)*)*)
(*color1=Green;color2=Green;color3=Green;color4=Green;color5=Green;*)
(**)
(*(*Usiamo delle variabili per ogni dado per mostrare se sia bloccato o meno*)*)
...
(**)
(*(*Con 5 variabili booleane impostiamo quali dadi teniamo per il punteggio e quali vogliamo rilanciare*)*)
...
(*girato=0;*)
(**)
(*(*Creiamo la lista che verr\[AGrave] poi utilizzata all'interno della tabella per mostrare le varie combinazioni con i rispettivi bottoni, unendo le corrispettive logiche per il calcolo dei punteggi in base ai dadi che abbiamo in quel turno.*)
(*Ogni riga della tabella finale \[EGrave] composta da un bottone e da un numero, ma in questa lista ci sono solo i bottoni. I numeri sono inseriti pi\[UGrave] avanti nella variabile tabella. Ogni bottone \[EGrave] abilitato solo se quella combinazione non \[EGrave] ancora stata scelta in questa partita e, quando viene premuto, conferma il valore relativo. *)
(*La condizione presente nell'If controlla se i dadi sono stati tirati almeno una volta questo turno e se l'utente non ha ancora scelto una combinazione. Se questa condizione risulta falsa cliccare il bottone non ha alcun effetto: non vogliamo che gli utenti possano bloccare combinazioni "alla cieca". Se la condizione \[EGrave] vera, dopo aver bloccato il punteggio corrispondente a quella combinazione il programma attraverso la funzione reset aggiorna il turno e sistema tutte le variabili per poter ricominciare a tirare i dadi.*)*)
...
(**)
(*(*Disegniamo ora i 5 dadi con le corrispettive logiche di gestione del valore libero o bloccato. Ogni dado \[EGrave] un bottone che cambia colore tra il rosso (bloccato) e il verde (libero)*)*)
(**)
(*(*Dado 1*)*)
...
(**)
(*(*Tabella dei punteggi con combinazioni e calcolo del punteggio per i dadi usciti in quel turno. Qui vengono dichiarati i punteggi abbinati ai bottoni che abbiamo descritto prima in listCombinations. Ogni valore \[EGrave] verde (il valore non \[EGrave] stato ancora bloccato) o rosso (\[EGrave] stato bloccato il valore) a seconda del suo stato. Allo stesso modo controlliamo che la combinazione non sia gi\[AGrave] stata scelta per cambiare il valore del punteggio.*)*)
...
(**)
(*(*Definiamo un bottone per svuotare la tabella e ricominciare il gioco con un altro nome, pulendo le celle di output create*)*)
...
(**)
(*(*Resettiamo la maggior parte delle variabili per ripristinare i valori inziali*)*)
...
(**)
(*(*Bottone che fa ricominciare la partita senza cambiare nome*)*)
...
(**)
(*(*Bottone che mostra gli high score: quando un punteggio viene salvato viene aggiunto al file locale scelto in precedenza che viene recuperato da questo pulsante e stampato a schermo. Prima di stampare viene cancellata l'ultima cella di output, in modo tale da non avere ripetizioni e di avere i punteggi pi\[UGrave] aggiornati*)*)
(*highButton=Button["Mostra High Score \|01f3ae",{NotebookDelete[NextCell[]],If[FindFile[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}]]!=$Failed,Print[Import[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}]]],Print["Non \[EGrave] ancora presente il file di High Score, salva il punteggio e poi clicca di nuovo qui!"]],If[FindFile[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}]]!="",Print[Import[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}]]],Print["Non \[EGrave] ancora presente il file di High Score, salva il punteggio e poi clicca di nuovo qui!"]]},Background->White,BaseStyle->{"GenericButton",12,Bold},ImageSize->{150,40}];*)
(**)
(*(*Bottone che apre una nuova finestra contenente il notebook che spiega le regole del gioco*)*)
...
(**)
(*(*Bottone per il salvataggio del punteggio. Diventa cliccabile solo dopo aver giocato una partita intera, ovvero quando tutte le combinazioni sono state riempite. Quando il pulsante viene cliccato, la funzione crea di default il file dove saranno salvati i punteggi (se gi\[AGrave] non esiste) e aggiunge il punteggio realizzato al file. Se invece il file esiste gi\[AGrave], semplicemente viene aggiunto il punteggio. La funzione funzCreaStringa che fa la maggior parte del lavoro verr\[AGrave] spiegata pi\[UGrave] in basso, alla sua definizione*)*)
(*saveButton=Button["Salva Punteggio \|01f4be",{ If[AllTrue[listK,TrueQ],{If[FindFile[ToString[fileScore]]!="",Export[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}],StringJoin["----------HIGH SCORES----------\n",StringRiffle[funzCreaStringa[prev,{ToExpression[Total[listPoints]], "-", ToString[nome]}],"\n","\t"]]],Null],If[FindFile[ToString[fileScore]]!=$Failed,Null,Export[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}],StringJoin["----------HIGH SCORES----------\n",ToString[Total[listPoints]],"\t","-","\t",ToString[nome],"\n"]]]},Null],flagSave = False },Enabled->Dynamic[AllTrue[listK,TrueQ] && flagSave == True],Background->White,BaseStyle->{"GenericButton",12,Bold},ImageSize->{150,40}];*)
(**)
(*(*Bottone che permette di chiudere il gioco, cancellando tutto ci\[OGrave] che \[EGrave] stato stampato*)*)
...
(**)
(*(*Questo comando permette di scegliere il nome del file dove salvare i punteggi*)*)
(*fileScore=ToString[saveFile];*)
(**)
...
(**)
(*(*Le funzioni check servono per controllare quanti punti un particolare set di dadi fa in una determinata categoria. Ci sono 14 funzioni check: 13 per le combinazioni e una per il bonus(la funzione per il bonus \[EGrave] scritta direttamente nella tabella). Tutte le funzioni restituiscono il numero di punti adatto secondo le regole di yahtzee oppure 0 se il pattern non viene riconosciuto.*)*)
(**)
(*(*Siccome le prime sei sono molto simili tra loro spieghiamo solo la prima: checkAces controlla quanti 1 ci sono nel set di dadi passato come parametro e ne restituisce la somma *)*)
...
(**)
(*(*checkFourOfAKind \[EGrave] simile alla precedente, ma qui controlliamo che 4 dadi siano uguali, questo vuol dire che il restante dado \[EGrave] o minore o maggiore degli altri risultando solamente in 2 pattern*)*)
...
(*Return[sum];];*)
(**)
(*(*checkSmallStraight funziona un po' diversamente rispetto alle altre funzioni: ci sono solo 3 possibili combinazioni che risultano in uno small straight: "1,2,3,4" | "2,3,4,5" | "3,4,5,6", ma il quinto dado pu\[OGrave] essere uno qualunque tra 1 e 6. Usando OrderlessPattern possiamo ignorare l'ordine e concentrarci sul contenuto*)*)
...
(**)
(*(*checkLargeStraight: ordinando i dadi ci sono solo due pattern possibili*)*)
(*checkLargeStraight[dices_]:=Module[{sum},If[MatchQ[dices,{1,2,3,4,5}|{2,3,4,5,6}],sum=40,sum=0];Return[sum]];*)
(**)
(*(*Yahtzee si fa quando tutti e 5 i dadi hanno lo stesso valore. Serve solamente un semplice pattern e una condizione per imporre che il valore dei dadi non \[EGrave] 0*)*)
(*checkYahtzee[dices_]:=Module[{sum},If[MatchQ[dices,{x_,x_,x_,x_,x_}/;x!=0],sum=50,sum=0];Return[sum];];*)
(**)
(*(*Chance \[EGrave] sempre disponibile e ritorna la somma dei dadi*)*)
(*checkChance[dices_]:=Module[{},Return[Total[dices]];];*)
(**)
(*(*Questa funzione prende come parametri una lista di punteggi ordinati e un punteggio nella forma {punteggio(int), - ,nome(string)} e restituisce una lista ordinata di liste che rappresentano un punteggio, compresa quella in input*)*)
(*funzCreaStringa[listPar_,punt_] := Module[{},temp = listPar; Return[ReverseSort[AppendTo[temp,punt]]];];*)
(**)
(*(*prev crea una lista di punteggi seguendo il seguente ordine: apre il file per il salvataggio dell'high score e fa un import del contenuto riga per riga in una lista, StringSplit trasforma queste righe in liste di parole, di queste liste di parole cancelliamo la prima che sar\[AGrave] sempre l'intestazione del file ("----------HIGH SCORE----------"), resta una lista di liste di punteggi, ogni primo valore viene trasformato in numero perch\[EGrave] \[EGrave] il punteggio e a questo punto possiamo ordinare la lista di liste risultante in ordine decrescente*)*)
(*prev := Module[{},Return[ReverseSort[MapAt[ToExpression,{All,1}]@Drop[StringSplit[Import[FileNameJoin[{NotebookDirectory[],ToString[fileScore]}],{"Text","Lines"}]],1]]]];*)
(**)
(*(*utente restituisce il nome preso in input. Per via della funzione di sort che usiamo in funzCreaStringa gli spazi non sono permessi, quindi vengono eliminati prima di ritornare l'input. Se l'utente esce dalla schermata di input o non inserisce un nome gli viene assegnato il nome "Sconosciuto" dal sistema*)*)
...
(**)
(*(*seme viene chiamato quando impostiamo il seed come prima istruzione di YAHTZEE. Se in input abbiamo qualcosa di valido (una stringa o un intero per esempio), viene usato come seme, altrimenti viene usato il tempo in millisecondi dal 1 Gennaio 1970 grazie a UnixTime[]*)*)
(*seme := Module[{ret},seed=InputString["Se vuoi usare un seme per il generatore casuale inseriscilo qui. In caso contrario lascia vuota la casella o chiudila con la X in alto a destra o col bottone Cancel."];*)
(*If[ToString[seed]=="$Canceled"||ToString[seed]==""||StringMatchQ[ToString[seed],Whitespace],ret = UnixTime[],ret = seed];Return[ret];];*)
(**)
(*(*Questa funzione viene utilizzata per scegliere il nome del file dove salvare i punteggi*)*)
(*saveFile := Module[{ret},file=InputString["Se vuoi salvare i punteggi in un file diverso da quello di default (punteggi.txt), inserisci il nome del file (senza estensione '.txt') qui. In caso contrario lascia vuota la casella o chiudila con la X in alto a destra o col bottone Cancel."];*)
(*If[ToString[file]=="$Canceled"||ToString[file]==""||StringMatchQ[ToString[file],Whitespace],ret = "punteggi.txt",ret = StringJoin[ToString[file],".txt"]];Return[ret];];*)
(**)
(*End[]*)
(**)
(*EndPackage[]*)
